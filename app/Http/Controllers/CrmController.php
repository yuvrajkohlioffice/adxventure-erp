...
class CrmController extends Controller
{
    ...
    public function data(Request $request)
    {
        $user = auth()->user();
        # 1 | Query 
        $query = Lead::with(['category', 'Followup', 'countries', 'user', 'AssignedUser', 'invoice']);
        ...
        return DataTables::of($query)
            ->editColumn('client_info', function ($lead) {
                ...
                $edit = '';
                if ($lead->status != 1) {
                    $edit = "<span class='badge text-dark' style='cursor:pointer' onclick=\"EditLead(
                        $lead->id,
                        '".addslashes($lead->name)."',
                        '".addslashes($lead->email)."',
                        '".addslashes($lead->country)."',
                        '".addslashes($lead->phone)."',
                        '".addslashes($lead->city)."',
                        '".addslashes($lead->client_category)."',
                        '".addslashes($lead->website)."',
                        '".addslashes($lead->domian_expire)."',
                        '".addslashes($lead->lead_status)."',
                        '".addslashes($lead->lead_source)."',
                        '".addslashes($lead->ref_name)."',
                        '".$lead->assigned_user_id."'
                    )\">Edit</span>";                          
                }

                $delayBadge = '';
                $lastFollowup = $lead->followup->last();
               ...
               return "<div class='order-md-1'>
                    ...
                    . (!empty($lead->email) ? "<small><a href='mailto:{$lead->email}'><i class='bi bi-envelope'></i> {$lead->email}</a></small><br>" : "")

                    . "<small><i class='bi bi-bag-plus'></i> Create Date: {$lead->created_at->format('d-m-y H:i:s')}</small>"
                    . ($daysDiff > 1 ? "<span class='badge bg-secondary'>{$daysDiff} Days Ago</span>" : "") . "<br>"
                    . $delayBadge .
                "</div>";

            })
            ->addColumn('service', function ($lead) {
                $lead = EncodingHelper::sanitizeUtf8($lead);
                if (!empty($lead->project_category)) {
                    $ids = json_decode($lead->project_category, true);
                    $names = ProjectCategory::whereIn('id', $ids)->pluck('name')->toArray();
            ...
            ->addColumn('location', function ($lead) {
                $lead = EncodingHelper::sanitizeUtf8($lead);
                $country = $lead->countries->nicename ?? 'N/A';
                $city = $lead->city ?? 'N/A';
                return "<strong class='lead-country' data-id='{$lead->id}' data-country='" . e($country) . "' style='cursor:pointer'><i class='bi bi-geo-alt-fill'></i>{$country}</strong><br>
                <small class='lead-city' data-id='{$lead->id}' data-city='{$city}' style='cursor:pointer'>($city)</small>";
            })
            ->addColumn('followup', function ($lead) {
                $lead = EncodingHelper::sanitizeUtf8($lead);
                $html = "<a class='btn btn-primary btn-sm' onclick=\"Followup({$lead->id}, '{$lead->name}', '{$lead->phone}', 1)\">Follow up";
                if ($lead->followup->count() >= 1) {
                    $html .= " ({$lead->followup->count()})";
                }
                $html .= "</a><br>";

                if ($lead->followup->count() >= 1) {
                    $last = $lead->followup->last();
                    $html .= "<small>(Last Follow-up: {$last->created_at->format('d-m-y H:i:s')})</small><br>
                            <small>Reason: {$last->reason}</small><br>";
                }

                $delayed = $lead->Followup->where('delay', 1)->count();
                if ($delayed >= 1) {
                    $html .= "<small class='badge bg-danger'>Delay: $delayed</small><br>";
                }
            ...
            ->addColumn('quotation', function ($lead) {
                $lead = EncodingHelper::sanitizeUtf8($lead);
                $html = '<a class="btn btn-sm btn-outline-warning " onclick="SendMessage('.$lead->id.')" data-bs-toggle="tooltip" data-bs-placement="top" title="Send Portfolio"><i class="bi bi-chat-dots"></i> Send Portfolio</a>  <br> ';
                $html .= '<a class="btn btn-sm btn-outline-primary mt-2" onclick="SendProposal('.$lead->id.')"  data-bs-toggle="tooltip" data-bs-placement="top" title="Send Proposal"><i class="bi bi-file-text"></i> Send Proposal</a>  <br>';
                if ($lead->quotation == 1) {
                    $html .= '<a href="'.route('crm.prposel.mail.view', ['leadId' => $lead->id]).'" class=" mt-2 btn btn-sm btn-outline-success" data-bs-toggle="tooltip" data-bs-placement="top" title="Resend Quotation"><i class="bi bi-file-earmark-arrow-up"></i> Resend Quotation </a><br>
                            <small>(Send Date: '.Carbon::parse($lead->quotation_date)->format('d-m-y H:i:s').')</small>';
                } else {
                    $html .= '<a href="'.route('crm.quotation.client', ['id' => $lead->id]).'" class="mt-2 btn btn-sm btn-outline-success " data-bs-toggle="tooltip" data-bs-placement="top" title="Send Quotation"><i class="bi bi-file-earmark-arrow-up"></i> Send Quotation </a>';
                }
                return $html;
            })
            ->addColumn('assigned_info', function ($lead) {
                $lead = EncodingHelper::sanitizeUtf8($lead);
                return "<small>Created by : <strong>" . ($lead->user->name ?? 'N/A') . "</strong></small><br>
                        <small>Assigned by: <strong>" . ($lead->user->name ?? 'N/A') . "</strong></small><br>
                        <small>Assigned User: <strong>" . ($lead->assignd_user->name ?? 'N/A') . "</strong></small>";
            })
            ->addColumn('actions', function ($lead) {
                ...
                if ($lead->quotation == 1) {
                    ...
                    if ($lead->invoice) {
                        $menu .= "<li><a class='dropdown-item' onclick=\"MarkAsPaid({$lead->invoice->id}, {$lead->invoice->balance}, '{$lead->name}')\">Mark as Paid</a></li>";
                    }
                    $menu .= "<li><a class='dropdown-item' href='".route('crm.prposel.mail.view', ['leadId'=>$lead->id])."'>View Quotation</a></li>";
                    // $menu .= "<li><a class='dropdown-item' href='".route('crm.message.view', ['id'=>$lead->id])."'>View Messages/Email</a></li>";
                    $menu .= "</ul></div>";
                    return $menu;
                }
                return '-';
            ...
            ->rawColumns([
                'checkbox', 'client_info', 'service', 'location', 
                'followup', 'quotation', 'assigned_info', 'actions'
            ])
            ->addIndexColumn() 
            ->make(true);
        }
    ...
    private function handleRoleBasedLogic($user, $query)
    {
        ...
        if ($user && $user->hasRole(['BDE', 'Business Development Intern'])) {
            ...
            $data['today_cold_clients'] = Followup::where('reason','call back later')->orWhere('reason', 'Not pickup')->whereDate('created_at',Carbon::today())->distinct('lead_id')->count();
