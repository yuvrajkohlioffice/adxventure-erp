                    ...
                    <div class="card-body"> 
                        ...
                        <div class="row">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                &nbsp; &nbsp;
                                                <a href="{{url('/expenses')}}" id="resetButton" class="btn btn-danger btn-danger" >Reset</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="col-4 mt-3">
                                <h3 class="badge text-bg-success text-md" style="float:right;font-size:18px">Total Expenses: ₹ {{$totalExpenses}}</h3>
                            </div>
                        </div>
                            ...
                            <table class="table table-striped table-bordered text-center pt-2">
                                ...
                                <tbody>
                                    ...
                                    @foreach($expenses as $key => $expense)
                                        <tr  class="bg-light">
                                            <td scope="row"> {{ $expenses->firstItem() + $key}}. </td>
                                            <td>{{ $expense->name}}</td>
                                            <td>{{ $expense->description}}</td>
                                            <td>{{ $expense->type_name ?? 0}}</td>
                                            <td>{{ $expense->price}}</td>
                                            <td>{{ \Carbon\Carbon::parse($expense->date)->format('d-m-Y') }}</td>
                                            <td>
                                                <a class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#EditModel{{$expense->id}}">Edit</a>
                                                <a href="{{ route('expenses.destroy', ['expense' => $expense->id]) }}" class="btn btn-sm btn-danger delete-btn"
                                                    onclick="event.preventDefault(); deleteConfirmation('{{ route('expenses.destroy', ['expense' => $expense->id]) }}');">
                                                        Delete
                                                </a>
                                            </td>
                                        </tr>
                                         <!-- Expenses Edit Model  -->
                                        <div class="modal" id="EditModel{{$expense->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
                                            <div class="modal-dialog modal-lg">
                                                <div class="modal-content" style="top:150px">   
                                                    ...
                                                    <div class="modal-body">
                                                        <form  class="ajax-form" data-action="{{ route('expenses.update',['expense'=>$expense->id]) }}"  data-method="POST">
                                                            @csrf
                                                            @method('PUT')
                                                            <div class="mb-3">
                                                                <label for="exampleInputcategory1" class="form-label">Expense Name</label>
                                                                <input type="text" class="form-control" name="name" placeholder="Enter Expense Name" value="{{ $expense->name}}" required>
                                                            </div>
                                                            <div class="mb-3">
                                                                ...
                                                                <select class="form-control" name="type">
                                                                    ...
                                                                    @if(isset($typs))
                                                                        @foreach($typs as $type)
                                                                            <option value="{{ $type->id }}" @if(isset($expense) && $expense->type == $type->id) selected @endif>
                                                                                {{ $type->name }}
                                                                            </option>
                                                                        @endforeach
                                                                    @endif
                                                            ...
                                                            <div class="mb-3">
                                                                ...
                                                                <input type="number" class="form-control" name="price" placeholder="Enter Expense Price" value="{{ $expense->price}}" required>
                                                            </div>
                                                            <div class="mb-3">
    ...
    <div class="modal" id="AddModel" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
        <div class="modal-dialog modal-lg">
            <div class="modal-content" style="top:150px">   
                ...
                <div class="modal-body">
                    <form  class="ajax-form" data-action="{{ route('expenses.store') }}"  data-method="POST">
                        @csrf
                        <div class="mb-3">
                            <label for="exampleInputcategory1" class="form-label">Expense Name</label>
    ...
    <div class="modal" id="TypeModel" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
        <div class="modal-dialog modal-lg">
            <div class="modal-content" style="top:150px">   
                ...
                <div class="modal-body">
                    <form  class="ajax-form" data-action="{{ route('expenses.type') }}"  data-method="POST">
                        @csrf
                        <div class="mb-3">
                            <label for="exampleInputcategory1" class="form-label">Expense Type</label>
