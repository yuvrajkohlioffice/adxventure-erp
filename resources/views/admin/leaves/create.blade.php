                ...
                <div class="card">
                    <div class="card-header">
                        <div class="col-12">
                            <div class="row">
                                <!-- User leave Filter -->
                                @if(Auth::user()->hasRole(['Super-Admin','Admin','Human Resources Executive','Marketing-Manager','Technology Manager','Digital Marketing Manager']))
                                <div class="col-3">
                                    <select class="form-select custom-select" name="user" id="filter-user">
                                        <option value="">Select User</option>
                                        @foreach($users as $user)
                                            <option value="{{ $user->id }}" {{ request('user') == $user->id ? 'selected' : '' }}>{{ $user->name }} ({{ $user->roles->pluck('name')->implode(', ') }})</option>
                                        @endforeach
                                    </select>
                                </div>
                                @endif
                                ...
                                <div class="col-3">
                                    <select class="form-select custom-select" name="type" id="filter-type">
                                        ...
                                        <option value="Half-day" {{ request('type') == 'Half-day' ? 'selected' : '' }}>Half Day</option>
                                    </select>
                                </div>
                                <!-- Leave Status Filter -->
                                <div class="col-3">
                                    <select class="form-select custom-select" name="status" id="filter-status">
                                        <option value="" {{ is_null(request('status')) ? 'selected' : '' }}>Select leave status..</option>
                                        <option value="1" {{ request('status') === '1' ? 'selected' : '' }}>Approved</option>
                                        <option value="2" {{ request('status') === '2' && !is_null(request('status')) ? 'selected' : '' }}>Un-Approved</option>
                                        <option value="0" {{ request('status') === '0' && !is_null(request('status')) ? 'selected' : '' }}>Pending</option>
                                    </select>
                                </div>
                            </div>
                        </div>  
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content" style="top:120px">
                ...
                <div class="modal-body">
                    <form autocomplete="off" data-method="POST" data-action="{{ url('leave/store') }}" id="ajax-form" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                                <div class="col-md-12">
                                ...
                                <div class="col-md-6 mt-3">
                                    ...
                                    <input type="date" class="form-control" id="from_date" name="from_date" min="{{ date('Y-m-d') }}"   placeholder="Enter From Date..">
                                    <small id="error-from_date" class="form-text error text-danger"></small>
                                </div>
                                <div class="col-md-6 mt-3">
                                    <label for="to_date"> To Date <span class="text-danger">*</span></label>
                                    <input type="date" class="form-control"  id="to_date" name="to_date"  min="{{ date('Y-m-d') }}" readonly  placeholder="Enter To Date..">
                                ...
                                <div class="col-md-12 mt-3">
                                    ...
                                    <textarea class="form-control" id="reason" name="reason" value="{{ old('subject') }}"  placeholder="Enter subject.."></textarea>
