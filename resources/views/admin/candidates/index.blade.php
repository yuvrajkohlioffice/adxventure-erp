<x-app-layout>
                                        ...
                                        <td>
                                            <small>
                                                <div class="form-check form-check-inline text-start">
                                                    <input class="form-check-input checkbox-action" type="checkbox" id="inlineCheckbox3" data-id="{{ $user->id }}" value="0" disabled {{ $user->interview == 0 ? 'checked' : '' }}>
                                                    <label class="form-check-label" for="inlineCheckbox3">No Given Interview</label>
                                                </div><br>
                                                <div class="form-check form-check-inline text-start">
                                                    <input class="form-check-input checkbox-action" type="checkbox" id="inlineCheckbox1" data-id="{{ $user->id }}" value="1" {{ $user->interview == 1 ? 'checked' : ($user->interview == 2 || $user->interview == 3 ? 'checked' : '') }}>
                                                    <label class="form-check-label" for="inlineCheckbox1">Hr Round</label>
                                                </div><br>
                                                <div class="form-check form-check-inline text-start">
                                                    <input class="form-check-input checkbox-action" type="checkbox" id="inlineCheckbox2" data-id="{{ $user->id }}" value="2" {{ $user->interview == 2 || $user->interview == 3 ? 'checked' : '' }}>
                                                    <label class="form-check-label" for="inlineCheckbox2">Technical Round</label>
                                                </div><br>
                                                <div class="form-check form-check-inline text-start">
                                                    <input class="form-check-input checkbox-action" type="checkbox" id="inlineCheckbox4" data-id="{{ $user->id }}" value="3" {{ $user->interview == 3 ? 'checked' : '' }}>
                                                    <label class="form-check-label" for="inlineCheckbox4">Interview Crack</label>
                                                </div><br>
                                            </small>
                                        </td>
                                        <td>
                                            @if($user->interview == 3 && $user->intern != 1)
                                                <a class="btn btn-primary btn-sm" onclick="GenrateOfferLetter({{$user->id}})">Genrate Offer letter</a>
                                                @if($user->offer_letter)
                                                 <a href="{{ asset($user->offer_letter) }}" target="_blank" class="btn btn-warning btn-sm">View</a>
                                                 @endif
                                            @else
                                                <a href="{{ asset($user->offer_letter) }}" target="_blank" class="btn btn-danger btn-sm">Candidate Not Eligibile</a>
                                            @endif
                                        </td>
                                        <th>
                                            @if($user->offer_letter)
                                                ...
                                                @else
                                                <button  class="btn btn-sm btn-success" onclick="Approve({{$user->id}})">Approve</button>
                                                @endif
                                            @elseif($user->intern == 1)
                                                @if($user->status == 1)
                                                <button  class="btn btn-sm btn-success">Employee Approved</button>
                                                @else
                                                <button  class="btn btn-sm btn-success" onclick="Approve({{$user->id}},1)">Approve</button>
                                                @endif
                                            @else
                                                @if(Auth::user()->hasRole(['Super-Admin', 'Admin']))
                                                <a href="{{ route('candidates.destroy', ['candidate' => $user->id]) }}" class="btn btn-sm btn-danger delete-btn"
                                                    onclick="event.preventDefault(); deleteConfirmation('{{ route('candidates.destroy', ['candidate' => $user->id]) }}');">
                                                        Delete
                                                </a>
                                                @endif
                                            @endif
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content" style="top:120px">
                ...
                <div class="modal-body">
                    <form id="ajax-form" data-action="{{ route('candidates.store') }}" data-method="POST">
                        @csrf
                        <div class="row">
                            <div class="col-12 ">
    ...
    <div class="modal" id="AddModel" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content" style="top:120px">
                ...
                <div class="modal-body">
                    <form class="ajax-form" data-action="{{route('offer.letter') }}" data-method="POST">
                        @csrf
    ...
    <div class="modal" id="approve" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content" style="top:120px">
                ...
                <div class="modal-body">
                    <form class="ajax-form" data-action="{{ route('add.employee') }}" data-method="POST" >
                        @csrf
                        <div class="row">
    ...
    <Script>
        function GenrateOfferLetter(id){
            $.ajax({
                url: '{{ route("genrate.offer.letter") }}',
                type: 'POST',
                data: {
                    id: id,
                    _token: '{{ csrf_token() }}'
