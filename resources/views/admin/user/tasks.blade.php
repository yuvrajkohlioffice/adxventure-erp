<x-app-layout>
    ...
    <div class="pagetitle">
        ...
        <h1>All Assign Tasks @if(request()->userData) || User Task : {{ request()->userData->name }}  @endif </h1>
        <nav>
            <ol class="breadcrumb">
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        <div class="row mt-5">
                            <div class="col-md-9">
                               <form id="ajaxFormData" style="margin-bottom:20px;">
                                    ...
                                    <div class="row">
                                        <div class="col-md-5"> 
                                            <input type="date" format="d-m-Y" class="form-control start_date" id="start_date" name="start_date" placeholder="Select date" value="{{ request()->start_date ?? '' }}" />
                                ...
                                <div class="row">
                                    <div class="col-xxl-4 col-md-4">
                                        <div class="card info-card bg-primary">
                                            ...
                                            <p style="font-size:30px;font-weight:700;"  id="totalTaskShow" class="text-white" >{{ request()->totalTask ?? '0' }}</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-xxl-4 col-md-4">
                                        <div class="card info-card bg-danger">
                                            <div style="padding:0px 20px !important;"  class="card-body">
                                                ...
                                                <p style="font-size:30px;font-weight:700;" id="totalPendingShow" class="text-white" >{{ request()->pendingTask ?? '0' }}</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-xxl-4 col-md-4">
                                        <div class="card info-card bg-success">
                                            ...
                                            <p style="font-size:30px;font-weight:700;"  id="totalComplatingShow" class="text-white" >{{ request()->doneTask ?? '0' }}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        @if(isset($projectsTasksCount) && count($projectsTasksCount) > 0)
                            <ul class="nav nav-tabs" id="myTab" role="tablist">
                                <!-- Project tabs loop -->
                                @foreach($projectsTasksCount as $project)
                                    <li class="nav-item">
                                        <button class="projectButton nav-link" id="{{ $project['project_id'] }}-tab" 
                                            data-tab="{{ $project['project_id'] }}" 
                                            data-filter="project" 
                                            data-project="{{ $project['project_id'] }}"
                                            type="button" role="tab" aria-selected="false"> 
                                            {{ ucfirst($project['name']) }} ({{$project['task_count']}})<!-- Corrected 'nmae' to 'name' -->
                                        </button>
                                    </li>
                                @endforeach
                            </ul>
                        ...
                        @if(isset($data) && $data->count() > 0 )
                            <table id="myTable" class="table table-striped">
                                ...
                                <tbody>
                                    ...
                                    @include('admin.tasks.tabledata', ['data' => $data])
                                </tbody>
                            </table>
                        @else
                            <h4 class="py-5">
    ...
    <form autocomplete="off" data-method="POST" data-action="{{ route('report.user.store') }}" id="send-ajax-form-report" enctype="multipart/form-data">
        @csrf
    ...
    <div class="modal" id="OtherReportModal" tabindex="-1" aria-labelledby="ViewDetailsModal" aria-hidden="true">
        <div class="modal-dialog modal-xl modal-dialog-centered">
            <div class="modal-content">
                ...
                <form action="{{route('report.other')}}" method="POST" id="otherTaskReport">
                    @csrf
...
<script>
    ...
    //callender
    $(function() {
        const dailyReport = {!! json_encode($dailyReport) !!};
    ...
    $(document).on('click', '.projectButton', function(event) {
        ...
        var id = "{{ $id ?? '' }}";
        if (id) {
            var href = "{{ route('project.task', ':id') }}";
                ...
                if(response.projectData){
                    $('#projectData').append(`
                        <div class="col-md-12">
                            ...
                            data-href="{{ url('task/send/GenerateReport/${response.projectData.id}') }}">
    ...
    function TaskData(filterParams = {}){
            ...
            @if(request()->userId)
                filterParams.userId = "{{ request()->userId }}";
            @endif
            @if(request()->project)
                filterParams.project = "{{ request()->project }}";
                filterParams.projectId = "yes";
            @endif
        {{-- @if(request()->start_date)
            filterParams.start_date = "{{ request()->start_date }}";
        @endif--}}
            $.ajax({
            url: "{{ route('project.task.ajax') }}", 
            type: "GET",
            data: filterParams,
            success: function (response) {
    ...
    $('#totalTask').val({{ request()->totalTask ?? '0' }})
    $('#PendingTask').val({{ request()->pendingTask ?? '0' }})
    $('#completeTask').val({{ request()->doneTask ?? '0' }})
